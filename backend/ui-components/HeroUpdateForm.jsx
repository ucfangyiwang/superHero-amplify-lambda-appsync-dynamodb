...
export default function HeroUpdateForm(props) {
  ...
  const resetStateValues = () => {
    ...
    setDurability(cleanValues.durability);
    setPower(cleanValues.power);
    setCombat(cleanValues.combat);
    setImage(cleanValues.image);
    setIsSave(cleanValues.isSave);
    setErrors({});
  };
  const [heroRecord, setHeroRecord] = React.useState(heroModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Hero, idProp)
        : heroModelProp;
      setHeroRecord(record);
    };
    queryData();
  }, [idProp, heroModelProp]);
    ...
    >
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || heroModelProp)}
